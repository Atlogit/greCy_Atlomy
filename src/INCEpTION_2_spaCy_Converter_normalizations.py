...
import unicodedata
#def clean_text_NFKD(text: str) -> str:
...
# "grc_ud_proiel_trf" is the name of the OLD spaCy models pipeline for ancient Greek NLP. Created by Jacobo Myerston, with the aristoBERTo tranformer model.
# "grc_proiel_trf" is the name of the NEW spaCy models pipeline for ancient Greek NLP. Created by Jacobo Myerston, with the aristoBERTo tranformer model.
...
# create DocBin object
        ...
        for sentence in cas.select("de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Sentence"):
            doc = nlp.make_doc(sentence.get_covered_text())
            print (doc)
            ...
            for token in cas.select_covered('webanno.custom.CategoryType', sentence):
                ...
                if token.value("Value") is not None and token is not None:
                    ...
                    span = doc.char_span(start, end, label=token.value("Value"), alignment_mode='expand')
                    print(span)
        ...
        lemma_dict_NFKD = {unicodedata.normalize("NFKD",key): unicodedata.normalize("NFKD",value) for key, value in lemma_dict.items()}
        lemma_dict_NFKC = {unicodedata.normalize("NFKC",key): unicodedata.normalize("NFKC",value) for key, value in lemma_dict.items()}
        
        with open(path_to_cas + "/INCEpTION_Data/" + "{0}_lemma_dict_NFKD.txt".format(file[:-4]), "w", encoding="utf-8") as f:
            for key, value in lemma_dict_NFKD.items():
                f.write(key + "\t" + value + "\n")
        f.close()
        
        with open(path_to_cas + "/INCEpTION_Data/" + "{0}_lemma_dict_NFKC.txt".format(file[:-4]), "w", encoding="utf-8") as f:
            for key, value in lemma_dict_NFKC.items():
                f.write(key + "\t" + value + "\n")
        f.close()
